...
import {
  ...
  computeCommissionRate, filterVideoStatsByPeriod, computeVideoStatsMetrics, computeEngagement, computeRpm,
  buildPostingBuckets, buildDailyPerformanceBuckets, formatBucketLabel, buildVideoPerformance,
...
import {
  SectionCard, StatTile, BarRow, LegendDot, SettlementBar, PeriodSelector, CurrencySelector, DailyPerformanceChart,
...
export const AnalyticsView: React.FC = () => {
  ...
  const commissionRate = computeCommissionRate(metrics.revenueTotal, metrics.gmvTotal);

  // Main chart: revenue + GMV + videos published, daily (or monthly for long histories — see
  // bucketGranularityForPeriod). Container-wide (not scoped to a TikTok handle) since this sits
  // above the account selector further down; videoStats here is deliberately the unfiltered
  // per-container set, matching how GMV/revenue above are also container-wide, not handle-wide.
  const { granularity: dailyPerfGranularity, buckets: dailyPerfBuckets } = useMemo(
    () => buildDailyPerformanceBuckets(currencyOrders, videoStats, period),
    [currencyOrders, videoStats, period]
  );
  ...
  const { granularity: postingGranularity, buckets: postingBuckets } = useMemo(
    () => buildPostingBuckets(usernameFilteredStats, period),
    [usernameFilteredStats, period]
  );
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {!hasOrdersData ? (
            ...
            <>
              {/* Stat tiles: GMV, Comisión, % de comisión sobre GMV — the chart right below reads off the same two numbers */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.6rem' }}>
                ...
                <StatTile
                  ...
                  value={metrics.gmvTotal > 0 ? `${(commissionRate * 100).toFixed(1)}%` : '—'}
                />
              </div>

              {/* Daily performance: GMV + settled revenue (one money scale) and videos published (its own scale) */}
