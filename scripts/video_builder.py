...
def main():
    ...
    # 5. Ensure there is enough raw footage to cover the target duration.
    #    If the total original footage is too short, duplicate clips cyclically.
    ...
    # 6. Water-filling: every clip gets a fair, equal share of the target duration.
    #    A clip shorter than its fair share is kept whole; the time it can't use
    #    is redistributed among the remaining (longer) clips. This guarantees all
    #    clips make it into the final video instead of a few long ones crowding
    #    out the short ones under a single global trim ratio.
    clip_durations = [get_media_duration(c) for c in final_clips]
    remaining_indices = list(range(len(final_clips)))
    remaining_budget = target_video_duration
    desired_durations = [0.0] * len(final_clips)

    while remaining_indices:
        share = remaining_budget / len(remaining_indices)
        fits_whole = [i for i in remaining_indices if clip_durations[i] <= share]
        if not fits_whole:
            for i in remaining_indices:
                desired_durations[i] = share
            break
        for i in fits_whole:
            desired_durations[i] = clip_durations[i]
            remaining_budget -= clip_durations[i]
            remaining_indices.remove(i)

    logger.info(f"Water-filled target durations per clip: {[round(d, 2) for d in desired_durations]}")

    # 7. Process each video: if trimming down to its share would cut away more
    #    than half the clip, speed it up 2x first and trim what's still needed
    #    from that sped-up footage. Otherwise, trim symmetrically from the edges.
    filter_concat = ""
    ffmpeg_inputs = ""

    for i, clip in enumerate(final_clips):
        original_duration = clip_durations[i]
        desired_duration = desired_durations[i]
        cut_fraction = 1 - (desired_duration / original_duration) if original_duration > 0 else 0

        output_trim = os.path.join(temp_dir, f"trim_clip_{i}.mp4")

        if cut_fraction > 0.5:
            # More than half would be lost by trimming alone: speed up 2x first,
            # then trim the remainder symmetrically from the sped-up footage.
            sped_duration = original_duration / 2
            surplus_time = sped_duration - desired_duration
            start_time = max(surplus_time / 2, 0)
            cmd_trim = (
                f'ffmpeg -y -i "{clip}" -vf "setpts=0.5*PTS,trim=start={start_time}:duration={desired_duration},'
                f'setpts=PTS-STARTPTS,scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920" '
                f'-pix_fmt yuv420p -an "{output_trim}"'
            )
            logger.info(f"Clip {i}: {original_duration:.2f}s -> {desired_duration:.2f}s ({cut_fraction*100:.0f}% cut, 2x sped up first)")
        else:
            # Stripping Logic: trim equal amounts from the start and end
            surplus_time = original_duration - desired_duration
            start_time = surplus_time / 2
            cmd_trim = (
                f'ffmpeg -y -ss {start_time} -t {desired_duration} -i "{clip}" '
                f'-vf "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920" -pix_fmt yuv420p -an "{output_trim}"'
            )
            logger.info(f"Clip {i}: {original_duration:.2f}s -> {desired_duration:.2f}s ({cut_fraction*100:.0f}% cut)")
